...
static double lookup_ref_throughput(bool checkpoint, bool new_average);
static double measure_ref_throughput(bool checkpoint, bool new_average);
...
int main(int argc, char **argv)
{
    ...
    double min_new_throughput = -1;
    double max_new_throughput = -1;


#if !REF_ONLY

    double ref_new_throughput;
...
#if !REF_ONLY
    /*
     * Get benchmark throughput with new average
     */
    ref_new_throughput = measure_ref_throughput(checkpoint, true);
    min_new_throughput = ref_new_throughput *
        (checkpoint ? MIN_SPEED_RATIO_CHECKPOINT : MIN_SPEED_RATIO);
    max_new_throughput = ref_new_throughput *
        (checkpoint ? MAX_SPEED_RATIO_CHECKPOINT : MAX_SPEED_RATIO);

    if (verbose > 0) {
        printf("Throughput targets: min=%.0f, max=%.0f, benchmark=%.0f\n",
               min_new_throughput, max_new_throughput, ref_new_throughput);
    }
#endif
    ...
    if (errors == 0) {
        ...
        p2_new = (1.0 - UTIL_WEIGHT) *
            compute_scaled_score(avg_mm_geom_throughput,
                min_new_throughput, max_new_throughput);
...
static double lookup_ref_throughput(bool checkpoint, bool new_average) {
    ...
    char *tput_filename = new_average ? NEW_THROUGHPUT_FILE : THROUGHPUT_FILE;
    ...
    FILE *tfile = fopen(tput_filename, "r");
    if (tfile == NULL) {
    ...
    if (tput == 0.0) {
        fprintf(stderr, "Warning: Could not find CPU '%s' benchmark '%s' in throughput file '%s'\n",
                cpu_type, bench_type, tput_filename);
    }
 ...
 */
static double measure_ref_throughput(bool checkpoint, bool new_average) {
    double ltput = lookup_ref_throughput(checkpoint, new_average);
    if (ltput > 0)
        return ltput;
    char buf[MAXLINE];
    char cmd[MAXLINE];
