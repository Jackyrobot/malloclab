...
$save_file_new = "./throughputs-new.txt";
...
if ($lookup_ok == 1) {
    ...
    $avg_new = read_throughput_file($save_file_new);
    if (($avg > 0.0) && ($avg_new > 0.0)) {
        print "Callibration: CPU type $cpu_info, benchmark $bench, throughput $avg, new throughput $avg_new\n";
        exit(0);
    }
}
...
for (my $i = 0; $i < $N; $i += 1) {
    ...
    my ($val, $val_new) = split / /, $sval;
    $val = $val * 1.0;
    $val_new = $val_new * 1.0;

    push (@values, $val);
    push (@values_new, $val_new);
...
$avg_new = &avg_without_outliers(@values_new);

# Save output to file
if ($save_results == 1) {
    &save_file_output($save_file, $avg);
    &save_file_output($save_file_new, $avg_new);
}
